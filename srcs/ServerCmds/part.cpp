...
void Server::part(const std::vector<std::string> &cmd, Client *const client) {
	...
	const std::vector<std::string> channels = Utils::splitString(cmd[1], CMD_ARG_SEPARATOR);
	for (std::vector<std::string>::const_iterator itCurrChanName = channels.begin(); itCurrChanName != channels.end(); itCurrChanName++) {
		...
		if (itM != _channels.end()) {
			...
			if (channel->isUserInChannel(client)) {
				...
				Utils::sendFormattedMessage(ERR_NOTONCHANNEL, client, *itCurrChanName);
		} else
			Utils::sendFormattedMessage(ERR_NOSUCHCHANNEL, client, *itCurrChanName);
	}
}
